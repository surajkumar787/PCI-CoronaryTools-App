...
// Same as atomic_add but returns previous value !
static FORCEINLINE int32_t fetch_add(int32_t * ptr, int32_t value)
{
...
static FORCEINLINE int64_t atomic_add(int64_t * ptr, int64_t value)
{
    return (_InterlockedExchangeAdd64((long long *)ptr, (long long)value) + value);
}
...
static FORCEINLINE float atomic_add(volatile float * ptr, float addend)
{
    for (;;)
    {
        int32_t oldValue = *(int32_t *)ptr;
        float newValue = reinterpret_cast<float &>(oldValue) + addend;
        if (oldValue == atomic_compare_exchange((int32_t *)ptr, reinterpret_cast<int32_t &>(newValue), oldValue))
            return newValue;
    }
}
