...
class FilterSepKernel
{
    static double sinc(double a)
    {
        ...
        return fabs(a) < numeric_limits<double>::min() ? 1 : std::sin(Pi * a) / (Pi * a);
    }
public:
    ...
    enum
    {
        Symmetric_Auto = 0, Symmetric_Yes = 1, Symmetric_No = 2
    };

    static SelfType GenericKernel(double const * kernel, int kernelSize, double desiredNorm, int kernelDim = 1, int symmetric = Symmetric_Auto)
    {
        SelfType result(kernelDim);
        ...
        for (int d = 0; d < kernelDim; d++)
        {
            ...
            for (int k = 0; k < FilterSepKernel::MaxSize; k++)
            {
                ...
                {
                    int input_k = k - offset;
                    if (symmetric == Symmetric_Yes)
                    {
                        input_k = __min(input_k, FilterSepKernel::MaxSize - input_k - 1);
                    }

                    result.Values[d][k] = desiredNorm / norm * kernel[d * kernelSize + input_k];
                }
                else
                {
                    result.Values[d][k] = 0;
...
template <int HALFKERSIZE>
struct FilterSepFunctor<HALFKERSIZE, true, Is16vec16>
{
    template <int I, typename T_IN1, typename T_IN2>
    FORCEINLINE void operator()(Is16vec16 & result, T_IN1 const & vecData, T_IN2 const & rKernel)
    {
        if (I == 0)
            result = mul_hrs(rKernel[0], vecData.at<HALFKERSIZE>());
        else
    ...
    template <typename T_IN1, typename T_IN2>
    FORCEINLINE Is16vec16 operator()(T_IN1 const & vecData, T_IN2 const & rKernel)
    {
        Is16vec16 result;
        unroller<0, 1+HALFKERSIZE>::go(*this, result, vecData, rKernel);
        return result;
...
template <int HALFKERSIZE>
struct FilterSepFunctor<HALFKERSIZE, false, Is16vec16>
{
    template <int I, typename T_IN1, typename T_IN2>
    FORCEINLINE void operator()(Is16vec16 & result, T_IN1 const & vecData, T_IN2 const & rKernel)
    {
