...
#define OSC_SHUTTERS_NON_EXTREMA_SCALE 10
#define OSC_SHUTTERS_EDGENESS_SCALE 4
#define OSC_SHUTTER_NON_SHUTTER_MIN_LEVEL_RATIO 1.3f
#define OSC_PICTURE_BORDER_NORMALIZED_VARIANCE .001f
#define OSC_REFERENCE_SCALE_IW 1024
#define OSC_SHUTTER_SYMMETRICITYLIMIT 40

// Provided as a convenience function
// Detection of Shutters/Borders
int OscExtractShutters(short* InIma, int Iw, int Ih, COscRoiCoord *RoiCoord)
{
   int               res;
   ...
   int x, y, xMin, yMin, xMax, yMax, offset, lineConstant, columnConstant, symmetricityLimit, Err;
   short val;
   ...
   safety_margin = 0;//OSC_SHUTTERS_SAFETY_MARGIN * Iw / OSC_REFERENCE_SCALE_IW; 
   non_extrema_scale = OSC_SHUTTERS_NON_EXTREMA_SCALE * Iw / OSC_REFERENCE_SCALE_IW;
   edgeness_scale = OSC_SHUTTERS_EDGENESS_SCALE * Iw / OSC_REFERENCE_SCALE_IW;
   shutter_non_shutter_min_level_ratio = OSC_SHUTTER_NON_SHUTTER_MIN_LEVEL_RATIO;
   picture_border_normalized_variance = OSC_PICTURE_BORDER_NORMALIZED_VARIANCE;
   symmetricityLimit = OSC_SHUTTER_SYMMETRICITYLIMIT *  Iw / OSC_REFERENCE_SCALE_IW;
   ...
   // alloc memory
   OSC_CALLOC(col_sum, Iw);
   OSC_CALLOC(row_sum, Ih);
   OSC_CALLOC(col_sq_sum, Iw);
   OSC_CALLOC(row_sq_sum, Ih);
   OSC_CALLOC(col_gradient_sum, Iw);
   OSC_CALLOC(row_gradient_sum, Ih);
   OSC_MALLOC(tmp_array, OSC_MAX(Iw, Ih));

#if OSC_USE_SSE_GAUSSIAN
   if (Iw%4==0)
   {
      // necessary to copy picture in GauIma, since memory alignment cannot be guaranteed for InIma
      OSC_MEMCPY(GauIma, InIma, Iw*Ih);
    
      Err = BxIsoGaussFilterFastShort2D(GauIma, GauIma, (float) edgeness_scale, Iw, Ih);
   }
   else
      Err = BxIsoGaussFilterShort2D(InIma, GauIma, (float) edgeness_scale, Iw, Ih);
#else
   Err = BxIsoGaussFilterShort2D(InIma, GauIma, (float) edgeness_scale, Iw, Ih);
#endif
   if (Err != 0){
       free(GauIma);
       free(col_sum); free(row_sum); free(col_sq_sum); free(row_sq_sum);
       free(col_gradient_sum); free(row_gradient_sum); free(tmp_array);
       printf("OscExtractShutters: allocation error in BxIsoGaussFilterShort2D\n"); return -1;
   }
   ...
   OSC_MEMCPY(tmp_array, row_gradient_sum, Ih);
   ...
   OSC_MEMCPY(tmp_array, col_gradient_sum, Iw);
   ...
   OSC_MEMZERO(col_sum, Iw);
   OSC_MEMZERO(row_sum, Ih);
   for (i=yMin; i<=yMax; i++)
   {
      for (j=xMin; j<=xMax; j++)
      {
   ...
   for (i=yMin; i<=yMax; i++)
   {
      row_sq_sum[i] = (row_sq_sum[i]-OSC_SQ((float) row_sum[i])/Iw);
   }
	for (j=xMin; j<=xMax; j++)
   {
      col_sq_sum[j] = (col_sq_sum[j]-OSC_SQ((float) col_sum[j])/Ih);
   }
   ...
   for (j=xMin; j<=xMax; j++)
   {
      if (col_gradient_sum[j]>0 && j>xMin+2*edgeness_scale)
      {
         ...
         if (left_mean*shutter_non_shutter_min_level_ratio<right_mean && right_var>picture_border_normalized_variance*OSC_SQ(right_mean))
         {
            left_pos = j;
            break;
         }
   ...
   if (left_pos==xMin)
   {
	  for (j=xMin; j<=xMax; j++)
      {
         if (col_gradient_sum[j]!=0 && j>2*edgeness_scale)
         {
            ...
            if (left_var<=picture_border_normalized_variance*OSC_SQ(left_mean) && right_var>picture_border_normalized_variance*OSC_SQ(right_mean))
            {
               left_pos = j;
               break;
            }
   ...
   for (j=xMax; j>=xMin; j--)
   {
      if (j<Iw-1-2*edgeness_scale && col_gradient_sum[j]<0)
      {
         ...
         if (left_mean>right_mean*shutter_non_shutter_min_level_ratio && left_var>picture_border_normalized_variance*OSC_SQ(left_mean))
         {
            right_pos = j;
            break;
         }
   ...
   for (i=yMin; i<=yMax; i++)
   {
      if (row_gradient_sum[i]>0 && i>2*edgeness_scale)
      {
         ...
         if (top_mean*shutter_non_shutter_min_level_ratio<bottom_mean && bottom_var>picture_border_normalized_variance*OSC_SQ(bottom_mean))
         {
            top_pos = i;
            break;
         }
   ...
   if (top_pos==yMin)
   {
      for (i=yMin; i<=yMax; i++)
      {
         if (row_gradient_sum[i]!=0 && i>2*edgeness_scale)
         {
            ...
            if (top_var<=picture_border_normalized_variance*OSC_SQ(top_mean) && bottom_var>picture_border_normalized_variance*OSC_SQ(bottom_mean))
            {
               top_pos = i;
               break;
            }
   ...
   for (i=yMax; i>=yMin; i--)
   {
      if (i<Ih-1-2*edgeness_scale && row_gradient_sum[i]<0)
      {
         ...
         if (top_mean>bottom_mean*shutter_non_shutter_min_level_ratio && top_var>picture_border_normalized_variance*OSC_SQ(top_mean))
         {
            bottom_pos = i;
            break;
         }
   ...
   RoiCoord->XMin   = OSC_MIN(left_pos  +safety_margin, Iw);
   RoiCoord->XMax   = OSC_MAX(right_pos -safety_margin, 0);

   res = OSC_SHUTEXT_OK;
   if (RoiCoord->XMin >= RoiCoord->XMax){
		res = OSC_SHUTEXT_ERR_LIM;

		//Default values
		if (RoiCoord->XMin >= Iw/2){
			if (xMin < Iw/2)
				RoiCoord->XMin = OSC_MIN(xMin  +safety_margin, Iw);
			else
				RoiCoord->XMin = safety_margin;
		}
		if (RoiCoord->XMax < Iw/2){
			if (xMax >= Iw/2)
				RoiCoord->XMax = OSC_MAX(xMax-safety_margin, 0);
			else
		...
		//Default values
		if (RoiCoord->XMin >= Iw/2){
			if (res == OSC_SHUTEXT_OK)
				res = OSC_SHUTEXT_WARNING_LIM_ABOVE_HALF;
   ...
   RoiCoord->YMin   = OSC_MIN(top_pos   +safety_margin, Ih);
   RoiCoord->YMax   = OSC_MAX(bottom_pos-safety_margin, 0);
   ...
   if (RoiCoord->YMin >= RoiCoord->YMax){
		...
		if (RoiCoord->YMin >= Ih/2){
			if (yMin < Ih/2)
				RoiCoord->YMin = OSC_MIN(yMin+safety_margin, Ih);
			else
				RoiCoord->YMin = safety_margin;
		}
		if (RoiCoord->YMax < Ih/2){
			if (yMax >= Ih/2)
				RoiCoord->YMax = OSC_MAX(yMax-safety_margin, 0);
			else
