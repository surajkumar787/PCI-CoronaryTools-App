...
short BxFastRoundShort(double val)
{
...
int BxGaussInitOrder3(float sigma, double * pB, double * pb1, double * pb2, double * pb3, double * M)
{
	double m0 = 1.16680, m1 = 1.10783, m2 = 1.40586;
	double m1sq = m1*m1, m2sq = m2*m2;
	double B, b0, b1, b2, b3 ; 
	double q, qsq, scale, div; 
	...
	div = (1.0+b1-b2+b3)*(1.0-b1-b2-b3)*(1.0+b2+(b1-b3)*b3);
	scale = 1.0/div;
...
int BxAniGaussFilterFloat2D(float * in, float* out, float sigmax, float sigmay, int dimx, int dimy)
{
    ...
    if ( (v0 == NULL) || (v1 == NULL) || (v2 == NULL) || (v3 == NULL) || (vbound == NULL) ){
        if (v0 != NULL){ free(v0); }
        if (v1 != NULL){ free(v1); }
        if (v2 != NULL){ free(v2); }
        if (v3 != NULL){ free(v3); }
        if (vbound != NULL){ free(vbound); }
        printf("BxAniGaussFilterFloat2D: could not allocate one of the vis");
        return -1;
    }

	BxGaussInitOrder3(sigmax, &B, &b1, &b2, &b3, M) ; 
	...
	BxGaussInitOrder3(sigmay, &B, &b1, &b2, &b3, M) ; 
...
int BxAniGaussFilterFloat2DROI(float * in, float* out, float sigmax, float sigmay, int dimx, int dimy
                               , int xMin, int xMax, int yMin, int yMax)
{
    ...
    v0    = (double*) malloc( dimxROI * sizeof(double) ) ;
	v1    = (double*) malloc( dimxROI * sizeof(double) ) ;
	v2    = (double*) malloc( dimxROI * sizeof(double) ) ;
	v3    = (double*) malloc( dimxROI * sizeof(double) ) ;
	vbound  = (double*) malloc( dimxROI * sizeof(double) ) ;
    ...
    if ( (v0 == NULL) || (v1 == NULL) || (v2 == NULL) || (v3 == NULL) || (vbound == NULL) ){
        ...
        if (v3 != NULL){ free(v3); }
        if (vbound != NULL){ free(vbound); }
        printf("BxAniGaussFilterFloat2DROI: could not allocate one of the vis");
        return -1;
    }

	BxGaussInitOrder3(sigmax, &B, &b1, &b2, &b3, M) ; 

    memcpy(out, in,dimx*dimy);

    in = in + (yMin*dimx + xMin); out = out + (yMin*dimx + xMin);
	for (y=yMin; y<=yMax; y++)
	{
		bound = in[dimxROI-1] ;
		begin = out ; 
	...
	out -= ((yMax+1)*dimx+xMin); 

	BxGaussInitOrder3(sigmay, &B, &b1, &b2, &b3, M) ; 

	for (x=0; x<dimxROI; x++)
	{
		v1[x] = v2[x] = v3[x] = out[yMin*dimx+xMin+x] ;
		vbound[x] = out[yMax*dimx+xMin+x] ; 
	}

    out = out + (yMin*dimx + xMin);
	for (y=yMin; y<=yMax; y++)
	{
		for (x=0; x<dimxROI; x++)
		{
...
{
	return BxAniGaussFilterFloat2D(in,out,sigma,sigma,dimx,dimy) ; 
}

int BxIsoGaussFilterFloat2DROI(float * in, float* out, float sigma, int dimx, int dimy, int xMin, int xMax, int yMin, int yMax)
{
	return BxAniGaussFilterFloat2DROI(in,out,sigma,sigma,dimx,dimy, xMin, xMax, yMin, yMax); 
}
...
{
	return BxAniGaussFilterShort2D(in,out,sigma,sigma,dimx,dimy) ; 
}
...
{
	return BxAniGaussFilterFastShort2D(in,out,sigma,sigma,dimx,dimy) ; 
}
...
int BxAniGaussFilterFloat2D_SSE(float * in, float* out, float sigmax, float sigmay, int dimx, int dimy)
{
    ...
    if ( (v0 == NULL) || (v1 == NULL) || (v2 == NULL) || (v3 == NULL) || (vbound == NULL) ){
        ...
        if (v3 != NULL){ free(v3); }
        if (vbound != NULL){ free(vbound); }
        printf("BxAniGaussFilterFloat2D_SSE: could not allocate one of the vis");
        return -1;
    }
...
{
	return BxAniGaussFilterFloat2D_SSE(in, out, sigma, sigma, dimx, dimy) ;
}
...
int BxAniGaussFilterFloat3D(float * in, float* out, float sigmax, float sigmay, float sigmaz, int dimx, int dimy, int dimz)
{
	...
	BxGaussInitOrder3(sigmaz, &B, &b1, &b2, &b3, M) ; 
...
{
	return BxAniGaussFilterFloat3D(in,out,sigma,sigma,sigma,dimx,dimy,dimz) ; 
}
...
{
	return BxAniGaussFilterShort3D(in,out,sigma,sigma,sigma,dimx,dimy,dimz) ; 
}
...
{
	return BxAniGaussFilterFloat3D_SSE(in, out, sigma, sigma, sigma, dimx, dimy, dimz) ;
}

/*int BxAniGaussFilterShort2D_SSE(short * in, short* out, float sigmax, float sigmay, int dimx, int dimy)
{
	int x, y ; 
	double M[9] ;
	double bound ; 
...
int BxIsoGaussFilterShort2D_SSE(short * in, short* out, float sigma, int dimx, int dimy)
{
	return BxAniGaussFilterShort2D_SSE(in, out, sigma, sigma, dimx, dimy) ;
}*/