...
int CBDExtractShutters(CCBD * This, short* InIma, int IwOri, int IhOri, int * ShutterPos)
{
   ...
   GauIma = (short*)calloc(IwOri*IhOri, sizeof(short));
	if (GauIma == NULL){
		ErrStartErrorReportFromLeaf(This->ErrHdl, "CBDExtractShutters", CBD_ERR_MEM_ALLOC
									, "Allocation of GauIma impossible");
		return CBD_ERR_MEM_ALLOC;
	}

   safety_margin = 0;//CBD_SHUTTERS_SAFETY_MARGIN * IwOri / CBD_REFERENCE_SCALE_IW; 
   non_extrema_scale = CBD_SHUTTERS_NON_EXTREMA_SCALE * IwOri / CBD_REFERENCE_SCALE_IW;
   edgeness_scale = CBD_SHUTTERS_EDGENESS_SCALE * IwOri / CBD_REFERENCE_SCALE_IW;
   shutter_non_shutter_min_level_ratio = CBD_SHUTTER_NON_SHUTTER_MIN_LEVEL_RATIO;
   picture_border_normalized_variance = CBD_PICTURE_BORDER_NORMALIZED_VARIANCE;
   symmetricityLimit = CBD_SHUTTER_SYMMETRICITYLIMIT *  IwOri / CBD_REFERENCE_SCALE_IW;
   ...
   col_sum = (int *) calloc(IwOri, sizeof(col_sum[0]));
	if (col_sum == NULL){
		ErrStartErrorReportFromLeaf(This->ErrHdl, "CBDExtractShutters", CBD_ERR_MEM_ALLOC
									, "Allocation of col_sum impossible");
        free(GauIma);
		return CBD_ERR_MEM_ALLOC;
	}
   row_sum = (int *) calloc(IhOri, sizeof(row_sum[0]));
	if (row_sum == NULL){
		ErrStartErrorReportFromLeaf(This->ErrHdl, "CBDExtractShutters", CBD_ERR_MEM_ALLOC
									, "Allocation of row_sum impossible");
   ...
   col_sq_sum = (float *) calloc(IwOri, sizeof(col_sq_sum[0]));
	if (col_sq_sum == NULL){
		ErrStartErrorReportFromLeaf(This->ErrHdl, "CBDExtractShutters", CBD_ERR_MEM_ALLOC
									, "Allocation of col_sq_sum impossible");
   ...
   row_sq_sum = (float *) calloc(IhOri, sizeof(row_sq_sum[0]));
	if (row_sq_sum == NULL){
		ErrStartErrorReportFromLeaf(This->ErrHdl, "CBDExtractShutters", CBD_ERR_MEM_ALLOC
									, "Allocation of row_sq_sum impossible");
   ...
   col_gradient_sum = (int *) calloc(IwOri,sizeof(col_gradient_sum[0]));
	if (col_gradient_sum == NULL){
		ErrStartErrorReportFromLeaf(This->ErrHdl, "CBDExtractShutters", CBD_ERR_MEM_ALLOC
									, "Allocation of col_gradient_sum impossible");
   ...
   row_gradient_sum = (int *) calloc(IhOri,sizeof(row_gradient_sum[0]));
	if (row_gradient_sum == NULL){
		ErrStartErrorReportFromLeaf(This->ErrHdl, "CBDExtractShutters", CBD_ERR_MEM_ALLOC
									, "Allocation of row_gradient_sum impossible");
        free(GauIma); free(col_sum); free(row_sum); free(col_sq_sum); free(row_sq_sum); free(col_gradient_sum);
		return CBD_ERR_MEM_ALLOC;
	}
   tmp_array = (int *) malloc(CBD_MAX(IwOri, IhOri)*sizeof(tmp_array[0]));
	if (tmp_array == NULL){
		ErrStartErrorReportFromLeaf(This->ErrHdl, "CBDExtractShutters", CBD_ERR_MEM_ALLOC
									, "Allocation of tmp_array impossible");
        free(GauIma); free(col_sum); free(row_sum); free(col_sq_sum); free(row_sq_sum); free(col_gradient_sum); free(row_gradient_sum);
		return CBD_ERR_MEM_ALLOC;
	}

#if CBD_USE_SSE_GAUSSIAN
   if (IwOri%4==0)
   {
      // necessary to copy picture in GauIma, since memory alignment cannot be guaranteed for InIma
      memcpy(GauIma, InIma, IwOri*IhOri*sizeof(short));
    
      BxIsoGaussFilterFastShort2D(GauIma, GauIma, (float) edgeness_scale, IwOri, IhOri);
   }
   else
      BxIsoGaussFilterShort2D(InIma, GauIma, (float) edgeness_scale, IwOri, IhOri);
#else
   BxIsoGaussFilterShort2D(InIma, GauIma, (float) edgeness_scale, IwOri, IhOri);
#endif
   ...
   for (i=yMin; i<=yMax; i++)
   {
      row_sq_sum[i] = (row_sq_sum[i]-CBD_SQ((float) row_sum[i])/IwOri);
   }
	for (j=xMin; j<=xMax; j++)
   {
      col_sq_sum[j] = (col_sq_sum[j]-CBD_SQ((float) col_sum[j])/IhOri);
   }
   ...
   for (j=xMin; j<=xMax; j++)
   {
      if (col_gradient_sum[j]>0 && j>xMin+2*edgeness_scale)
      {
         ...
         if (left_mean*shutter_non_shutter_min_level_ratio<right_mean && right_var>picture_border_normalized_variance*CBD_SQ(right_mean))
         {
            left_pos = j;
            break;
         }
   ...
   if (left_pos==xMin)
   {
	  for (j=xMin; j<=xMax; j++)
      {
         if (j>2*edgeness_scale && col_gradient_sum[j]!=0)
         {
            int k, n_points;
            ...
            if (left_var<=picture_border_normalized_variance*CBD_SQ(left_mean) && right_var>picture_border_normalized_variance*CBD_SQ(right_mean))
            {
               left_pos = j;
               break;
            }
   ...
   for (j=xMax; j>=xMin; j--)
   {
      if (j<IwOri-1-2*edgeness_scale && col_gradient_sum[j]<0)
      {
         ...
         if (left_mean>right_mean*shutter_non_shutter_min_level_ratio && left_var>picture_border_normalized_variance*CBD_SQ(left_mean))
         {
            right_pos = j;
            break;
         }
   ...
   for (i=yMin; i<=yMax; i++)
   {
      if (i>2*edgeness_scale && row_gradient_sum[i]>0)
      {
         ...
         if (top_mean*shutter_non_shutter_min_level_ratio<bottom_mean && bottom_var>picture_border_normalized_variance*CBD_SQ(bottom_mean))
         {
            top_pos = i;
            break;
         }
   ...
   if (top_pos==yMin)
   {
      for (i=yMin; i<=yMax; i++)
      {
         if (row_gradient_sum[i]!=0 && i>2*edgeness_scale)
         {
            ...
            if (top_var<=picture_border_normalized_variance*CBD_SQ(top_mean) && bottom_var>picture_border_normalized_variance*CBD_SQ(bottom_mean))
            {
               top_pos = i;
               break;
            }
   ...
   for (i=yMax; i>=yMin; i--)
   {
      if (row_gradient_sum[i]<0 && i<IhOri-1-2*edgeness_scale)
      {
         ...
         if (top_mean>bottom_mean*shutter_non_shutter_min_level_ratio && top_var>picture_border_normalized_variance*CBD_SQ(top_mean))
         {
            bottom_pos = i;
            break;
         }
   ...
   ShutterPos[0]   = CBD_MIN(left_pos  +safety_margin, IwOri);
   ShutterPos[1]   = CBD_MAX(right_pos -safety_margin, 0);
   ...
   if (ShutterPos[0] >= ShutterPos[1]){
		...
		if (ShutterPos[0] >= IwOri/2){
			if (xMin < IwOri/2)
				ShutterPos[0] = CBD_MIN(xMin  +safety_margin, IwOri);
			else
				ShutterPos[0] = safety_margin;
		}
		if (ShutterPos[1] < IwOri/2){
			if (xMax >= IwOri/2)
				ShutterPos[1] = CBD_MAX(xMax-safety_margin, 0);
			else
   ...
   ShutterPos[2]   = CBD_MIN(top_pos   +safety_margin, IhOri);
   ShutterPos[3]   = CBD_MAX(bottom_pos-safety_margin, 0);
   ...
   if (ShutterPos[2] >= ShutterPos[3]){
		...
		if (ShutterPos[2] >= IhOri/2){
			if (yMin < IhOri/2)
				ShutterPos[2] = CBD_MIN(yMin+safety_margin, IhOri);
			else
				ShutterPos[2] = safety_margin;
		}
		if (ShutterPos[3] < IhOri/2){
			if (yMax >= IhOri/2)
				ShutterPos[3] = CBD_MAX(yMax-safety_margin, 0);
			else
